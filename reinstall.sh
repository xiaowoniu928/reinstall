...
继续 DD?'
    ...
    setos_almalinux() {
        ...
        if is_use_cloud_image; then
            # ci
            if is_in_china; then
                ci_mirror="https://mirror.nju.edu.cn/almalinux/$releasever/cloud/$elarch/images"
            else
                ci_mirror="https://repo.almalinux.org/almalinux/$releasever/cloud/$elarch/images"
            fi
            ci_image=$ci_mirror/AlmaLinux-$releasever-GenericCloud-latest.$elarch.qcow2
